                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        @if (Auth::user()->isAdmin())
                            ...
                            {{-- Tambahkan link kelola jadwal, portofolio dll. --}}
                        @elseif (Auth::user()->isUser())
                            <x-nav-link :href="route('my.bookings.index')" :active="request()->routeIs('my.bookings.index')">
                                {{ __('Pemesanan Saya') }}
                            </x-nav-link>
                            {{-- Tambahkan link untuk menemukan MC (akan menuju mc/1) --}}
                            <x-nav-link :href="route('mc.show', ['id' => 1])" :active="request()->routeIs('mc.show')"> {{-- Ganti 1 dengan ID MC dinamis nanti --}}
                                {{ __('Booking MC') }}
                            </x-nav-link>
                        @elseif (Auth::user()->isEditor())
                            ...
                            {{-- Tambahkan link manajemen user/konten --}}
                        @endif
                    @endauth
                </div>
            </div>
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                ...
                @elseif (Auth::user()->isUser())
                    ...
                    <x-responsive-nav-link :href="route('mc.show', ['id' => 1])" :active="request()->routeIs('mc.show')">
                        {{ __('Booking MC') }}
                    </x-responsive-nav-link>
                @elseif (Auth::user()->isEditor())
